...
pub fn desEncryptCbc(keyBytes: []const u8, iv: []const u8, inData: []const u8, outData: []u8) void {
    ...
    while (offset <= inData.len - 8) {
        ...
        block = mem.readIntSliceLittle(u64, cipher);
        i += 1;
        offset += 8;
    }
}

pub fn desDecryptCbc(keyBytes: []const u8, iv: []const u8, inData: []const u8, outData: []u8) void {
    assert(keyBytes.len == 8);
    assert(iv.len == 8);
    assert(inData.len % 8 == 0);
...
pub fn des3EncryptCbc(keyBytes: []const u8, iv: []const u8, inData: []const u8, outData: []u8) void {
    ...
    while (offset <= inData.len - 8) {
        ...
        des3EncryptEcb(keyBytes, buf[0..], out[0..]);
        for (out) |c, j| {
            outData[(i * 8) + j] = c;
        }
...
pub fn des3DecryptCbc(keyBytes: []const u8, iv: []const u8, inData: []const u8, outData: []u8) void {
    assert(keyBytes.len == 24);
    assert(iv.len == 8);
    assert(inData.len % 8 == 0);
    ...
    while (offset <= inData.len - 8) {
        ...
        des3DecryptEcb(keyBytes, buf[0..], out[0..]);
        for (out) |*p, j| {
            p.* ^= block[j];
        }
        for (out) |c, j| {
...
pub fn desEncryptEcb(keyBytes: []const u8, inData: []const u8, outData: []u8) void {
    ...
    var i: u64 = 0;
    var offset: u64 = 0;
    const key = mem.readIntSliceNative(u64, keyBytes[0..]);
    const keys = subkeys(key);
    while (offset <= inData.len - 8) {
        const plain: u64 = mem.readIntSliceBig(u64, inData[offset..(offset + 8)]);
...
pub fn des3EncryptEcb(keyBytes: []const u8, inData: []const u8, outData: []u8) void {
    ...
    var i: u64 = 0;
    var offset: u64 = 0;
    const keys1 = subkeys(mem.readIntSliceBig(u64, keyBytes[0..8]));
    const keys2 = subkeys(mem.readIntSliceBig(u64, keyBytes[8..16]));
    const keys3 = subkeys(mem.readIntSliceBig(u64, keyBytes[16..]));
    while (offset <= inData.len - 8) {
        const plain: u64 = mem.readIntSliceBig(u64, inData[offset..(offset + 8)]);
        var cipher = desRounds(keys1[0..], plain, true);
        cipher = desRounds(keys2[0..], mem.readIntSliceNative(u64, cipher), false);
        cipher = desRounds(keys3[0..], mem.readIntSliceNative(u64, cipher), true);
        for (cipher) |c, j| {
            outData[(i * 8) + (7 - j)] = c;
        }
        i += 1;
...
pub fn des3DecryptEcb(keyBytes: []const u8, inData: []const u8, outData: []u8) void {
    ...
    while (offset <= inData.len - 8) {
        ...
        cipher = desRounds(keys2[0..], mem.readIntSliceNative(u64, cipher), true);
        cipher = desRounds(keys1[0..], mem.readIntSliceNative(u64, cipher), false);
        for (cipher) |c, j| {
            outData[(i * 8) + (7 - j)] = c;
        }
        i += 1;
